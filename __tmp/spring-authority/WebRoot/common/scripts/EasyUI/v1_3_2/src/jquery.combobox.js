...
//		if (!opts.url) return;
	...
	function create(target){
        ...
        if(opts.selectOnly&&!opts.multiple){
            $(target).combo('textbox').bind("blur",function(){
                if($(target).combo('panel').find('div.combobox-item-selected').length==0)$(target).combo("clear");
            })
        }
	}
	...
	$.fn.combobox.defaults = $.extend({}, $.fn.combo.defaults, {
		...
		keyHandler: {
			...
			down: function(){
                if($(this).combo('panel').find('div.combobox-item').is(':visible')){
                    selectNext(this);
                }else{
                    $($(this).next().find("span[class='combo-arrow']:last")).trigger("click");
                }
            },
			enter: function(){
                if($(this).combo('panel').find('div.combobox-item').is(':visible')){
