...
(function (JQuery7) {
    ...
    JQuery7.fn.gantt = function (options) {
        ...
        var core = {
            ...
            elementFromPoint: function (x, y) {
                ...
                if (JQuery7.browser.msie) {
                    x -= JQuery7(document).scrollLeft();
                    y -= JQuery7(document).scrollTop();
                } else {
            ...
            create: function (element) {
                ...
                if (typeof settings.source !== "string") {
                    ...
                    JQuery7.getJSON(settings.source, function (jsData) {
                        element.data = jsData;
                        core.init(element);
                    });
                }
            ...
            render: function (element) {
                var content = JQuery7('<div class="fn-content"/>');
                var JQuery7leftPanel = core.leftPanel(element);
                content.append(JQuery7leftPanel);
                var JQuery7rightPanel = core.rightPanel(element, JQuery7leftPanel);
                var mLeft, hPos;

                content.append(JQuery7rightPanel);
                content.append(core.navigation(element));

                var JQuery7dataPanel = JQuery7rightPanel.find(".dataPanel");

                element.gantt = JQuery7('<div class="fn-gantt" />').append(content);

                JQuery7(element).html(element.gantt);

                element.scrollNavigation.panelMargin = parseInt(JQuery7dataPanel.css("margin-left").replace("px", ""), 10);
                element.scrollNavigation.panelMaxPos = (JQuery7dataPanel.width() - JQuery7rightPanel.width());

                element.scrollNavigation.canScroll = (JQuery7dataPanel.width() > JQuery7rightPanel.width());

                core.markNow(element);
                core.fillData(element, JQuery7dataPanel, JQuery7leftPanel);

                if (settings.useCookie) {
                    var sc = JQuery7.cookie(this.cookieKey + "ScrollPos");
                    if (sc) {
                ...
                if (settings.scrollToToday) {
                    ...
                    if ((startPos > 0 && element.hPosition !== 0)) {
                        if (element.scaleOldWidth) {
                            mLeft = (JQuery7dataPanel.width() - JQuery7rightPanel.width());
                            hPos = mLeft * element.hPosition / element.scaleOldWidth;
                            hPos = hPos > 0 ? 0 : hPos;
                            JQuery7dataPanel.css({ "margin-left": hPos + "px" });
                            element.scrollNavigation.panelMargin = hPos;
                            element.hPosition = hPos;
                            element.scaleOldWidth = null;
                        } else {
                            JQuery7dataPanel.css({ "margin-left": element.hPosition + "px" });
                ...
                JQuery7dataPanel.css({ height: JQuery7leftPanel.height() });
                core.waitToggle(element, false);
                settings.onRender();
            },
            leftPanel: function (element) {
                /* Left panel */
                var ganttLeftPanel = JQuery7('<div class="leftPanel"/>')
                    .append(JQuery7('<div class="row spacer"/>')
                        .css("height", tools.getCellSize() * element.headerRows + "px")
                        .css("width", "100%"));

                var entries = [];
                JQuery7.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        entries.push('<div class="row name row' + i + (entry.desc ? '' : ' fn-wide') + '" id="rowheader' + i + '" offset="' + Math.ceil(i % settings.itemsPerPage * tools.getCellSize()) + '">');
            ...
            dataPanel: function (element, width) {
                var dataPanel = JQuery7('<div class="dataPanel" style="width: ' + width + 'px;"/>');
                /*
                 * Mouse wheel events
                 */
                ...
                dataPanel.click(function (e) {
                    ...
                    e.stopPropagation();
                    var corrX, corrY;
                    var leftpanel = JQuery7(element).find(".fn-gantt .leftPanel");
                    var datapanel = JQuery7(element).find(".fn-gantt .dataPanel");
                    switch (settings.scale) {
                        case "weeks":
            ...
            rightPanel: function (element, leftPanel) {
                ...
                switch (settings.scale) {
                    ...
                    case "hours":
                            ...
                            if (rm !== month) {
                                monthArr.push(
                                    ('<div class="row header month" style="width: '
                                        + Math.ceil(tools.getCellSize() * daysInMonth) + 'px"><div class="fn-label">'
                        ...
                        dataPanel.append(JQuery7('<div class="row"/>').html(dayArr.join("")));
                        dataPanel.append(JQuery7('<div class="row"/>').html(dowArr.join("")));
                        dataPanel.append(JQuery7('<div class="row"/>').html(horArr.join("")));
                    ...
                    case "weeks":
                        ...
                        for (var i = 0; i < range.length; i++) {
                            ...
                            if (rday.getMonth() !== month) {
                                monthArr.push(
                                    ('<div class="row header month" style="width:'
                                        + Math.ceil(tools.getCellSize() * daysInMonth)
                                        + 'px;"><div class="fn-label">'
                                        + settings.months[month]
                                        + '</div></div>'));
                                month = rday.getMonth();
                            ...
                            dayArr.push('<div class="row day wd" '
                                + ' id="' + rday.getWeekId() + '" offset="' + Math.ceil(i * tools.getCellSize()) + '" repdate="' + rday.genRepDate() + '"> '
                    ...
                    case 'months':
                        ...
                        for (var i = 0; i < range.length; i++) {
                            ...
                            daysInYear++;
                            monthArr.push('<div class="row day wd" id="dh-' + tools.genId(rday.getTime()) + '" offset="' + Math.ceil(i * tools.getCellSize()) + '" repdate="' + rday.genRepDate() + '">' + (1 + rday.getMonth()) + '</div>');
                ...
                return JQuery7('<div class="rightPanel"></div>').append(dataPanel);
            },
            ...
            navigation: function (element) {
                ...
                if (settings.navigate === "scroll") {
                    ganttNavigate = JQuery7('<div class="navigate" />')
                        .append(JQuery7('<div class="nav-slider" />')
                            .append(JQuery7('<div class="nav-slider-left" />')
                                .append(JQuery7('<span role="button" class="nav-link nav-page-back"/>')
                                    .html('&lt;')
                                    .click(function () {
                                        core.navigatePage(element, -1);
                                    }))
                                .append(JQuery7('<div class="page-number"/>')
                                    .append(JQuery7('<span/>')
                                        .html(element.pageNum + 1 + ' of ' + element.pageCount)))
                                .append(JQuery7('<span role="button" class="nav-link nav-page-next"/>')
                                    .html('&gt;')
                                    .click(function () {
                                        core.navigatePage(element, 1);
                                    }))
                                .append(JQuery7('<span role="button" class="nav-link nav-now"/>')
                                    .html('&#9679;')
                                    .click(function () {
                                        core.navigateTo(element, 'now');
                                    }))
                                .append(JQuery7('<span role="button" class="nav-link nav-prev-week"/>')
                                    .html('&lt;&lt;')
                                    .click(function () {
                                ...
                                .append(JQuery7('<span role="button" class="nav-link nav-prev-day"/>')
                                    .html('&lt;')
                                    .click(function () {
                                        if (settings.scale === 'hours') {
                            ...
                            .append(JQuery7('<div class="nav-slider-content" />')
                                .append(JQuery7('<div class="nav-slider-bar" />')
                                    .append(JQuery7('<a class="nav-slider-button" />')
                                    )
                            ...
                            .append(JQuery7('<div class="nav-slider-right" />')
                                .append(JQuery7('<span role="button" class="nav-link nav-next-day"/>')
                                    .html('&gt;')
                                    .click(function () {
                                        if (settings.scale === 'hours') {
                                            core.navigateTo(element, tools.getCellSize() * -4);
                                ...
                                .append(JQuery7('<span role="button" class="nav-link nav-next-week"/>')
                                    .html('&gt;&gt;')
                                    .click(function () {
                                        if (settings.scale === 'hours') {
                                            core.navigateTo(element, tools.getCellSize() * -8);
                                ...
                                .append(JQuery7('<span role="button" class="nav-link nav-zoomIn"/>')
                                    .html('&#43;')
                                    .click(function () {
                                        core.zoomInOut(element, -1);
                                    }))
                                .append(JQuery7('<span role="button" class="nav-link nav-zoomOut"/>')
                                    .html('&#45;')
                                    .click(function () {
                                        core.zoomInOut(element, 1);
                                    }))
                    ...
                    JQuery7(document).mouseup(function () {
                    ...
                    ganttNavigate = JQuery7('<div class="navigate" />')
                        ...
                        .append(JQuery7('<span role="button" class="nav-link nav-begin"/>')
                            .html('&#124;&lt;')
                            .click(function () {
                        ...
                        .append(JQuery7('<span role="button" class="nav-link nav-end"/>')
                            .html('&gt;&#124;')
                            .click(function () {
                ...
                return JQuery7('<div class="bottom"/>').append(ganttNavigate);
            },
            createProgressBar: function (days, cls, desc, label, dataObj) {
                var cellWidth = tools.getCellSize();
                var barMarg = tools.getProgressBarMargin() || 0;
                var bar = JQuery7('<div class="bar"><div class="fn-label">' + label + '</div></div>')
                    .addClass(cls)
                    .css({
                ...
                if (desc) {
                    bar
                        .mouseover(function (e) {
                            var hint = JQuery7('<div class="fn-gantt-hint" />').html(desc);
                            JQuery7("body").append(hint);
                            hint.css("left", e.pageX);
                            hint.css("top", e.pageY);
                            hint.show();
                        })
                        .mouseout(function () {
                            JQuery7(".fn-gantt-hint").remove();
                        })
                        .mousemove(function (e) {
                            JQuery7(".fn-gantt-hint").css("left", e.pageX);
                            JQuery7(".fn-gantt-hint").css("top", e.pageY + 15);
                        });
                }
                bar.click(function (e) {
                    e.stopPropagation();
                    settings.onItemClick(JQuery7(this).data("dataObj"));
                });
                return bar;
            },
            markNow: function (element) {
                switch (settings.scale) {
                    case "weeks":
                        ...
                        JQuery7(element).find(':findweek("' + cd + '")').removeClass('wd').addClass('today');
                        break;
                    case "months":
                        JQuery7(element).find(':findmonth("' + new Date().getTime() + '")').removeClass('wd').addClass('today');
                        break;
                    default:
                        var cd = Date.parse(new Date());
                        cd = (Math.floor(cd / 36400000) * 36400000);
                        JQuery7(element).find(':findday("' + cd + '")').removeClass('wd').addClass('today');
                        break;
                }
            },
            ...
            fillData: function (element, datapanel, leftpanel) {
                ...
                JQuery7.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        ...
                        JQuery7.each(entry.values, function (j, day) {
                            var _bar = null;
                            switch (settings.scale) {
                                case "hours":
                                    ...
                                    // find row
                                    var topEl = JQuery7(element).find("#rowheader" + i);
                                ...
                                case "weeks":
                                    ...
                                    var from = JQuery7(element).find("#" + dtFrom.getWeekId());

                                    var cFrom = from.attr("offset");

                                    var to = JQuery7(element).find("#" + dtTo.getWeekId());
                                    var cTo = to.attr("offset");
                                ...
                                case "months":
                                    ...
                                    var from = JQuery7(element).find("#dh-" + tools.genId(dtFrom.getTime()));
                                    var cFrom = from.attr("offset");
                                    var to = JQuery7(element).find("#dh-" + tools.genId(dtTo.getTime()));
                                    var cTo = to.attr("offset");
                                    var dl = Math.round((cTo - cFrom) / tools.getCellSize()) + 1;
                            ...
                            var JQuery7l = _bar.find(".fn-label");
                            if (JQuery7l && _bar.length) {
                                var gray = invertColor(_bar[0].style.backgroundColor);
                                JQuery7l.css("color", gray);
                            } else if (JQuery7l) {
                                JQuery7l.css("color", "");
                            }
                        });
            ...
            navigateTo: function (element, val) {
                var JQuery7rightPanel = JQuery7(element).find(".fn-gantt .rightPanel");
                var JQuery7dataPanel = JQuery7rightPanel.find(".dataPanel");
                JQuery7dataPanel.click = function () {
                    alert(arguments.join(""));
                };
                var rightPanelWidth = JQuery7rightPanel.width();
                var dataPanelWidth = JQuery7dataPanel.width();

                switch (val) {
                    case "begin":
                        JQuery7dataPanel.animate({
                            "margin-left": "0px"
                    ...
                    case "end":
                        ...
                        break;
                    case "now":
                        if (!element.scrollNavigation.canScroll || !JQuery7dataPanel.find(".today").length) {
                            return false;
                        }
                        var max_left = (dataPanelWidth - rightPanelWidth) * -1;
                        var cur_marg = JQuery7dataPanel.css("margin-left").replace("px", "");
                        var val = JQuery7dataPanel.find(".today").offset().left - JQuery7dataPanel.offset().left;
                        val *= -1;
                        if (val > 0) {
                            val = 0;
            ...
            navigatePage: function (element, val) {
                if ((element.pageNum + val) >= 0 && (element.pageNum + val) < Math.ceil(element.rowsNum / settings.itemsPerPage)) {
                    core.waitToggle(element, true, function () {
                        element.pageNum += val;
                        element.hPosition = JQuery7(".fn-gantt .dataPanel").css("margin-left").replace("px", "");
            ...
            zoomInOut: function (element, val) {
                core.waitToggle(element, true, function () {
                    ...
                    if ((zoomIn && JQuery7.inArray(scale, scales) < JQuery7.inArray(settings.minScale, scales))
                        || (!zoomIn && JQuery7.inArray(scale, scales) > JQuery7.inArray(settings.maxScale, scales))) {
                        core.init(element);
                        return;
                    }
                    ...
                    var JQuery7rightPanel = JQuery7(element).find(".fn-gantt .rightPanel");
                    var JQuery7dataPanel = JQuery7rightPanel.find(".dataPanel");
                    element.hPosition = JQuery7dataPanel.css("margin-left").replace("px", "");
                    element.scaleOldWidth = (JQuery7dataPanel.width() - JQuery7rightPanel.width());

                    if (settings.useCookie) {
                        JQuery7.cookie(this.cookieKey + "CurrentScale", settings.scale);
                        // reset scrollPos
                        JQuery7.cookie(this.cookieKey + "ScrollPos", null);
                    }
                    core.init(element);
                });
            },
            mouseScroll: function (element, e) {
                var JQuery7dataPanel = JQuery7(element).find(".fn-gantt .dataPanel");
                JQuery7dataPanel.css("cursor", "move");
                var bPos = JQuery7dataPanel.offset();
            ...
            sliderScroll: function (element, e) {
                var JQuery7sliderBar = JQuery7(element).find(".nav-slider-bar");
                var JQuery7sliderBarBtn = JQuery7sliderBar.find(".nav-slider-button");
                var JQuery7rightPanel = JQuery7(element).find(".fn-gantt .rightPanel");
                var JQuery7dataPanel = JQuery7rightPanel.find(".dataPanel");

                var bPos = JQuery7sliderBar.offset();
                var bWidth = JQuery7sliderBar.width();
                var wButton = JQuery7sliderBarBtn.width();
                ...
                if ((e.pageX >= bPos.left) && (e.pageX <= bPos.left + bWidth)) {
                    ...
                    pos = pos - wButton / 2;
                    JQuery7sliderBarBtn.css("left", pos);

                    mLeft = JQuery7dataPanel.width() - JQuery7rightPanel.width();

                    var pPos = pos * mLeft / bWidth * -1;
                    if (pPos >= 0) {
                        JQuery7dataPanel.css("margin-left", "0px");
                        element.scrollNavigation.panelMargin = 0;
                    } else if (pos >= bWidth - (wButton * 1)) {
                        JQuery7dataPanel.css("margin-left", mLeft * -1 + "px");
                        element.scrollNavigation.panelMargin = mLeft * -1;
                    } else {
                        JQuery7dataPanel.css("margin-left", pPos + "px");
            ...
            scrollPanel: function (element, delta) {
                ...
                if (_panelMargin > 0) {
                    element.scrollNavigation.panelMargin = 0;
                    JQuery7(element).find(".fn-gantt .dataPanel").css("margin-left", element.scrollNavigation.panelMargin + "px");
            ...
            synchronizeScroller: function (element) {
                if (settings.navigate === "scroll") {
                    ...
                    var JQuery7dataPanel = JQuery7rightPanel.find(".dataPanel");
                    var JQuery7sliderBar = JQuery7(element).find(".nav-slider-bar");
                    var JQuery7sliderBtn = JQuery7sliderBar.find(".nav-slider-button");

                    var bWidth = JQuery7sliderBar.width();
                    var wButton = JQuery7sliderBtn.width();

                    var mLeft = JQuery7dataPanel.width() - JQuery7rightPanel.width();
                    var hPos = 0;
                    if (JQuery7dataPanel.css("margin-left")) {
                        hPos = JQuery7dataPanel.css("margin-left").replace("px", "");
                    }
                    var pos = hPos * bWidth / mLeft - JQuery7sliderBtn.width() * 0.25;
                    pos = pos > 0 ? 0 : (pos * -1 >= bWidth - (wButton * 0.75)) ? (bWidth - (wButton * 1.25)) * -1 : pos;
                    JQuery7sliderBtn.css("left", pos * -1);
                }
            },
            repositionLabel: function (element) {
                setTimeout(function () {
                    var JQuery7dataPanel;
                    if (!element) {
                        JQuery7dataPanel = JQuery7(".fn-gantt .rightPanel .dataPanel");
                    } else {
                        var JQuery7rightPanel = JQuery7(element).find(".fn-gantt .rightPanel");
                        JQuery7dataPanel = JQuery7rightPanel.find(".dataPanel");
                    }

                    if (settings.useCookie) {
                        JQuery7.cookie(this.cookieKey + "ScrollPos", JQuery7dataPanel.css("margin-left").replace("px", ""));
                    }
                }, 500);
            },
            waitToggle: function (element, show, fn) {
                if (show) {
                    var eo = JQuery7(element).offset();
                    var ew = JQuery7(element).outerWidth();
                    var eh = JQuery7(element).outerHeight();

                    if (!element.loader) {
                        element.loader = JQuery7('<div class="fn-gantt-loader" style="position: absolute; top: ' + eo.top + 'px; left: ' + eo.left + 'px; width: ' + ew + 'px; height: ' + eh + 'px;">'
                            + '<div class="fn-gantt-loader-spinner"><span>' + settings.waitText + '</span></div></div>');
                    }
                    JQuery7("body").append(element.loader);
                    setTimeout(fn, 100);
        ...
        var tools = {
            ...
            getMaxDate: function (element) {
                var maxDate = null;
                JQuery7.each(element.data, function (i, entry) {
                    JQuery7.each(entry.values, function (i, date) {
            ...
            getCellSize: function () {
                if (!tools._getCellSize) {
                    JQuery7("body").append(
                        JQuery7('<div style="display: none; position: absolute;" class="fn-gantt" id="measureCellWidth"><div class="row"></div></div>')
                    );
                    tools._getCellSize = JQuery7("#measureCellWidth .row").height();
                    JQuery7("#measureCellWidth").empty().remove();
                }
                return Math.ceil(tools._getCellSize);
            },
            getRightPanelSize: function () {
                JQuery7("body").append(
                    JQuery7('<div style="display: none; position: absolute;" class="fn-gantt" id="measureCellWidth"><div class="rightPanel"></div></div>')
                );
                var ret = JQuery7("#measureCellWidth .rightPanel").height();
                JQuery7("#measureCellWidth").empty().remove();
                return ret;
            },
            ...
            getProgressBarMargin: function () {
                if (!tools._getProgressBarMargin) {
                    JQuery7("body").append(
                        JQuery7('<div style="display: none; position: absolute;" id="measureBarWidth" ><div class="fn-gantt"><div class="rightPanel"><div class="dataPanel"><div class="row day"><div class="bar" /></div></div></div></div></div>')
                    );
                    tools._getProgressBarMargin = parseInt(JQuery7("#measureBarWidth .fn-gantt .rightPanel .day .bar").css("margin-left").replace("px", ""), 10);
                    tools._getProgressBarMargin += parseInt(JQuery7("#measureBarWidth .fn-gantt .rightPanel .day .bar").css("margin-right").replace("px", ""), 10);
                    JQuery7("#measureBarWidth").empty().remove();
                }
        ...
        this.each(function () {
             ...
             */
            if (options) {
                JQuery7.extend(settings, options);
            }
            ...
            if (settings.useCookie) {
                var sc = JQuery7.cookie(this.cookieKey + "CurrentScale");
                if (sc) {
                    settings.scale = JQuery7.cookie(this.cookieKey + "CurrentScale");
                } else {
                    JQuery7.cookie(this.cookieKey + "CurrentScale", settings.scale);
                }
            }
